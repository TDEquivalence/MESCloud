...
SELECT
    ...
    s.reliability,
    (
        SELECT ARRAY_AGG(po_sub.code)
        FROM production_order po_sub
        WHERE cpo.id = po_sub.composed_production_order_id
    ) AS production_order_codes,
    po.input_batch,
    po.source,
    po.gauge,
    po.category,
...
LEFT JOIN (
    SELECT
        s.composed_production_order_id,
...
WHERE
    crpc.is_valid_for_production = true
GROUP BY
    cpo.id,
    cpo.created_at,
    cpo.code,
